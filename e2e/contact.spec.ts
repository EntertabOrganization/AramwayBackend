...
test.describe("Contact messages API", () => {
  test("public create, then protected list/update/delete round-trip", async ({ request }) => {
    ...
    const createRes = await request.post("/contact", {
    ...
    const listRes = await request.get("/contact?limit=1000");
    const list = await listRes.json();
    expect(list.data.some((m: { id: string }) => m.id === created.id)).toBe(true);

    const updateRes = await request.patch(`/contact/${created.id}`, { data: { status: "READ" } });
    expect(updateRes.ok()).toBeTruthy();
    expect((await updateRes.json()).status).toBe("READ");

    const deleteRes = await request.delete(`/contact/${created.id}`);
    expect(deleteRes.ok()).toBeTruthy();
    expect((await request.get(`/contact/${created.id}`)).status()).toBe(404);
  });

  test("rejects a create request missing required fields", async ({ request }) => {
    const res = await request.post("/contact", { data: { name: "No Email" } });
